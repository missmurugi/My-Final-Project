...
internal class ResidentialActivity : AppCompatActivity() {
    ...
    private fun uploadFile() {
        if (mImageUri != null) {
            ...
            mUploadTask = fileReference.putFile(mImageUri!!).addOnSuccessListener { taskSnapshot ->
                ...
                val upload = Upload(property_type,property_distance,property_bedrooms, taskSnapshot.storage.getDownloadUrl().toString(),System.currentTimeMillis().toString())
                val uploadId = mDatabaseRef!!.push().key
                mDatabaseRef!!.child(System.currentTimeMillis().toString() + "").setValue(upload)}.addOnFailureListener { e ->
                //Failure
    ...
    private fun openImagesActivity() {
        val intent = Intent(applicationContext, ViewresidentialsActivity::class.java)
